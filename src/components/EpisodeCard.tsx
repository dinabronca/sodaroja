...
export const EpisodeCard: React.FC<EpisodeCardProps> = ({ episode }) => {
  ...
  // Merge legacy + new format
  const links = episode.links || {};
  const embeds = episode.embeds || {};
  if (episode.spotifyUrl && !links.spotify) links.spotify = episode.spotifyUrl;
  if (episode.soundcloudUrl && !links.soundcloud) links.soundcloud = episode.soundcloudUrl;
  if (episode.spotifyEmbedUrl && !embeds.spotify) embeds.spotify = episode.spotifyEmbedUrl;
  if (episode.ivooxEmbedUrl && !embeds.ivoox) embeds.ivoox = episode.ivooxEmbedUrl;
            ...
            {formattedDate && !episode.isPremium && (
              <div className="absolute top-4 left-4 glass-fog px-3 py-1.5 rounded-sm z-20">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-soda-lamp text-xs">{formattedDate}</span>
                </div>
              </div>
            )}
      ...
      <AnimatePresence>
        {isExpanded && !episode.isPremium && (
          ...
          >
            ...
            >
              {/* Close */}
              <button onClick={() => setIsExpanded(false)} className="absolute top-4 right-4 z-50 w-10 h-10 bg-soda-night bg-opacity-80 rounded-full flex items-center justify-center text-soda-lamp hover:text-soda-glow transition-colors">
                <X size={20} />
              </button>
              ...
              <div className="p-6 space-y-6">
                ...
                {embeds.spotify && (
                  <div>
                    <h4 className="text-soda-accent text-xs tracking-wider mb-3">SPOTIFY</h4>
                    <div className="rounded-sm overflow-hidden"><iframe src={embeds.spotify} width="100%" height="152" frameBorder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy" /></div>
                  </div>
                )}
                {embeds.soundcloud && (
                  <div>
                    <h4 className="text-soda-accent text-xs tracking-wider mb-3">SOUNDCLOUD</h4>
                    <div className="rounded-sm overflow-hidden"><iframe src={embeds.soundcloud} width="100%" height="166" frameBorder="0" allow="autoplay" loading="lazy" /></div>
                  </div>
                )}
                {embeds.ivoox && (
                  <div>
                    <h4 className="text-soda-accent text-xs tracking-wider mb-3">IVOOX</h4>
                    <div className="rounded-sm overflow-hidden"><iframe src={embeds.ivoox} width="100%" height="200" frameBorder="0" loading="lazy" /></div>
                  </div>
                )}
                {embeds.applePodcasts && (
                  <div>
                    <h4 className="text-soda-accent text-xs tracking-wider mb-3">APPLE PODCASTS</h4>
                    <div className="rounded-sm overflow-hidden"><iframe src={embeds.applePodcasts} width="100%" height="175" frameBorder="0" sandbox="allow-forms allow-popups allow-same-origin allow-scripts allow-top-navigation-by-user-activation" allow="autoplay *;" loading="lazy" /></div>
                  </div>
                )}
